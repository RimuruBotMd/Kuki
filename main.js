(async () => {
	...
	const authFolder = `${opts._[0] || 'makemeow'}`
	const {
		state,
		saveCreds
	...
	async function connectionUpdate(update) {
		...
		if (update.receivedPendingNotifications) {
			...
			const message = `• *Info*: Sazumii Kemii successfully connected
◦ *Os*: ${os.platform()} ${os.release()}
◦ *Device*: ${deviceName}
◦ *Connected time*: ${new Date().toLocaleString()}`;

			conn.sendMessage(`120363023427805441@g.us`, {
				text: message
			});
		}
	}
